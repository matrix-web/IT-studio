...
// Инициализаяи события swipe для элемента mainElement
swipe(mainElement, {
    maxTime: 1000,
    minTime: 100,
    maxDist: 150,
...
function openMobileSidebar (event) {
    switch(event.detail.dir) {
        case 'right':
            sidebarElement.classList.add('blog-sidebar--active')
            document.body.classList.add('active')
        break
        case 'left':
            sidebarElement.classList.remove('blog-sidebar--active')
            document.body.classList.remove('active')
        break
    }
}

// Открытие боковой панели (сайдбара) свайпом по экрану
mainElement.addEventListener('swipe', openMobileSidebar)

// Удаление обработчика события swipe на настольных компьютерах
if (mediaQuery.matches) {
    console.log('Swipe event handler removed for desktop')
    mainElement.removeEventListener('swipe', openMobileSidebar)
}